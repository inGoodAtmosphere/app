  ...
  switch (req.method) {
    case 'POST':
      if (adress.length !== 17) {
        ...
        query = `select id from devices where Macadress=${dbQuery.escape(adressDecrypted
        )}`;
        checkIfAdressRegistered = await dbQuery(query)
          if (checkIfAdressRegistered.length === 0) {
            queryTest = `insert into devices(Macadress, Location_Latitude, Location_Longitude) values(${dbQuery.escape( adressDecrypted )}, ${dbQuery.escape(req.body.locationLatitude)}, ${dbQuery.escape(
