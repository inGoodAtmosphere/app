...
export default async (req, res) => {
  let query = '';
  let result = '';
  const checkIfAdressRegistered = {};
  const {
    query: { adress },
  } = req;
  switch (req.method) {
    case 'POST':
      query = `select id from devices where Macadress=${db.escape(adress.replace("U","F").replace("V","E").replace("W","D").replace("X","C").replace("Y","T").replace("Z","A"))}`;
