  ...
  const sendMail= async (recipient, subject,htmlContent , prefix, suffix)=>new Promise((resolve, reject)=>{
      ...
      if(!recipient) {
        const error = new Error("Recipient isn't set, if you see this error, let us know that we could fix this error") // creating an error to reject a promise,pssst eslint is my best friend at using good practices
        reject(error);
      }
      if(!subject) {
